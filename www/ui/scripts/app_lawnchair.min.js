...
this.element[0].addEventListener("webkitTransitionEnd",endAnimate,false);},animateWithClass:function(className,properties,cb){this.animate(properties,cb);}});var DataProvider=Class.extend({init:function(key){},get:function(cb){},getById:function(id,cb){},save:function(obj,cb){if(cb)cb();},remove:function(obj,cb){},removeAll:function(){}});var View=EventManager.extend({init:function(element){this.element=(typeof element==="string")?document.getElementById(element):element;},render:function(templateId,data){var tmp=template(templateId,data);this.element.innerHTML=tmp;},find:function(selector){return this.element.querySelector(selector);},findAll:function(selector){return this.element.querySelectorAll(selector);},remove:function(){this.element.innerHTML="";},show:function(){this.element.style.display="block";},hide:function(){this.element.style.display="none";},findParentWithAttribute:function(element,attribute){do{if(element.getAttribute(attribute))
return element;element=element.parentNode;}while(element)}});var TemplateView=View.extend({_render:function(templateId,data){}});var Model=Class.extend({init:function(initial){if(!this._data)this._data={};if(initial){extendObjStrict(this._data,initial);}
